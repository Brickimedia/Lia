 ...
 * @ingroup Skins
 */
class SkinLia extends SkinTemplate {

	protected static $bodyClasses = array( 'vector-animateLayout' );

	var $skinname = 'lia', $stylename = 'lia',
		$template = 'LiaTemplate', $useHeadElement = true;
 ...
 * @ingroup Skins
 */
class LiaTemplate extends BaseTemplate {
...
?>
		...
		<!-- content -->
		<div id="navCover"><div class="navCover"></div></div>
		<div id="content" class="mw-body">
			<a id="top"></a>
			<div id="mw-js-message" style="display:none;"<?php $this->html( 'userlangattributes' ) ?>></div>
			<?php if ( $this->data['sitenotice'] ): ?>
			<!-- sitenotice -->
			<div id="siteNotice"><div class="sitenotice-corner"></div><?php $this->html( 'sitenotice' ) ?></div>
			<!-- /sitenotice -->
			<?php endif; ?>
			<!-- buttons -->
			<span class="pageButtons">
			<!-- TROUBLE BUTTON -->
			<span class="button talk"><?php foreach ( $this->data['content_navigation']['namespaces'] as $key => $tab ) { ?>
			<?php echo $this->makeListItem( $key, $tab ); ?>
			<?php } ?></span>
			<!-- END TROUBLE BUTTON -->
			<span class="edit-button-group">
				<a accesskey="e" href="?action=edit" class="edit">
					Edit</a>
				<span class="drop">&#9660
					<ul class="menu" style="display:none;">
						<?php $this->renderNavigation( array( 'VIEWS', 'ACTIONS' ) ); ?><div <?php if ( count( $this->data['action_urls'] ) == 0 ) echo ' emptyPortlet'; ?>">
		<ul<?php $this->html( 'userlangattributes' ) ?>>
			<?php foreach ( $this->data['view_urls'] as $link ): if ( $link['id'] != 'ca-edit' && $link['id'] != 'ca-view' ): ?>
				<li<?php echo $link['attributes'] ?>><a href="<?php echo htmlspecialchars( $link['href'] ) ?>" <?php echo $link['key'] ?>><?php if ($link['text'] == '+') {$link['text'] = 'New';} echo htmlspecialchars( $link['text'] ) ?></a></li>
	...
	<ul><li class="not-link">
			...
			<a href="http://stories.brickimedia.org">Stories</a></li>
		</li><li class="">
			<a href="http://cuusoo.brickimedia.org">CUUSOO</a></li></ul></div>
			<!-- Code was here -->
...
<?php			} ?>
	<li id="pt-switch"><a id="swapTabs" title="Switch between tabs and buttons for page commands">Swap</a></li>
	</ul>
</div>
<?php
				break;
